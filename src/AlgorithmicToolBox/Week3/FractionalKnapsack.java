...
public class FractionalKnapsack {
    private static double getOptimalValue(int capacity, int[] values, int[] weights) {
        ...
        List<Integer> valuesL = new ArrayList<>(values.length);
        for (int i = 0; i < values.length; i++) {
            valuesL.add(values[i]);
        }

        List<Integer> weightsL = new ArrayList<>(weights.length);
        ...
        while (capacity > 0) {
            ...
            if (values.length == 1) {
                ...
                return maxLoot = maxLoot + ((double) valuesL.get(m) * amount) / weightsL.get(m);
            }
